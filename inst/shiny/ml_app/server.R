    ...
    function(input, output) {
        ...
        observeEvent(
            input$update, {
                withProgress(value = 0, {
                    responses_path <- system.file("extdata", "responses.rda", package = "multilex", mustWork = TRUE)
        ...
        output$vocabulary_plot_total <- renderPlot({
            if (input$vocabulary_longitudinal=="only") {
                ...
                vocabulary %>%
                    ...
                    labs(x = "Age (months)", y = "Total vocabulary size (%)",
        ...
        output$vocabulary_table <- DT::renderDataTable({
            ...
            vocabulary_table <- vocabulary %>%
                ...
                mutate_at(vars(matches("vocab")), ~scales::label_percent(accuracy = 0.01)(.)
                ) %>%
        ...
        output$participants_table <- DT::renderDataTable({
            participants %>%
                ...
                arrange(desc(index)) %>%
                select(id, id_db, code, time, study, version, randomisation, date_sent, call, comments) %>%
                DT::datatable(
                    rownames = FALSE,
                    width = "1000px",
                    height = "4000px",
                    style = "bootstrap",
                    colnames = c("ID", "ID (DB)", "Code", "Time", "Study", "Version", "Randomisation", "Sent", "Status", "Comments"),
                    filter = "top",
                    options = list(
                        pageLength = 30,
                        autoWidth = FALSE
